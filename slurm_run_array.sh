...
#
#   TARGET_ENV=HalfCheetah-v5 sbatch --array=0-29 slurm_run_array.sh
#   TARGET_ENV=dog-stand      sbatch --array=0-29 slurm_run_array.sh
#   TARGET_ENV=myoLegWalk-v0  sbatch --array=0-29 slurm_run_array.sh
#
# Optional: TOTAL_STEPS (default 1_000_000)
#
# n_envs (SB3 baselines' vec-env count) is set from SLURM_CPUS_PER_TASK,
# so it always matches the actual allocation below. ERL/SEMARL ignore
# n_envs and always run pop_size+1 workers (11 by default) regardless of -c: that's
...
#   TARGET_ENV=Ant-v5 sbatch --array=0-29 --cpus-per-task=16 slurm_run_array.sh
...
ALGORITHMS=(sac ppo td3 crossq erl semarl)
SEEDS=(0 1 2 3 4)
N_SEEDS=${#SEEDS[@]}
